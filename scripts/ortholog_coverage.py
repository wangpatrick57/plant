...
ortholog_pairs_file = sys.argv[1]
s1_nodes = set()
s2_nodes = set()
graphlet_distr = defaultdict(int)
total_nodes_num = 0

for line in open(ortholog_pairs_file, 'r'):
...
print(f'graphlet distribution')
# print('\n'.join(f'{id} {count}' for id, count in sorted_graphlet_distr))
print(f'species1 covers {len(s1_nodes)} nodes')
print(f'species2 covers {len(s2_nodes)} nodes')
