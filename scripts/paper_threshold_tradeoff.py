...
for sims_threshold in sims_thresholds:
    ...
    for max_indices in max_indiceses:
        all_seeds_list = find_seeds(s1_index, s2_index, ODVDirectory(get_odv_file_path(species1)), ODVDirectory(get_odv_file_path(species2)), SeedingAlgorithmSettings(sims_threshold=sims_threshold, max_indices=max_indices), print_progress=False)
